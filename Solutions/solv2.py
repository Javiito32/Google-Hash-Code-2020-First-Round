def solveHashCode (fileName):
    ...
    class library:
        ...
        def setBooksList(self):
            ...
            for i in _books:
                ...
                c += 1
            Fbooks = {k: v for k, v in sorted(Fbooks.items(), key=lambda item: item[1])}
    ...
    for i in libraries:
        if i.signed == False:
            _signTimes.update({i.libNum: i.signIn})

    _signTimes = {k: v for k, v in sorted(_signTimes.items(), key=lambda item: item[1])}
    ...
    while _signTimes != {}:
        ...
        order.append(k)
        qtty = libraries[k].perDay*(TotalDays-pastDays)
